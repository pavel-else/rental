...
class Request    
{
    ...
    public function __construct($app_id, $queue = null) {
        $this->app_id = $app_id;
        $this->queue = $queue;
    }
    ...
    public function rent_connect_DB()
    {
        require_once('./lib.db.php');
...
if ($dataJSON['cmd'] === 'login') {
    ...
    $app_id = Request::verifyToken($dataJSON['token']);

    if ($app_id) {
        $request = new Request($app_id, $dataJSON['queue']);
        $request->response();
    } else {
        exit;
        die;
